...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logger.error("Gagal menyimpan nilai esai", { route: "ujian/nilai-esai", ujianId, guruId: session.userId, ...errorContext(err) });
    url.search = `?error=${encodeURIComponent("Gagal menyimpan nilai, coba lagi")}`;
    return NextResponse.redirect(url, { status: 303 });
  }
}
