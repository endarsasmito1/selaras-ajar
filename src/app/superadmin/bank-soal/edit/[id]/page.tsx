...
import { getSoalGlobalById, getSemuaNamaMapelUnik } from "@/lib/data";
...
export default async function EditSoalGlobalPage({
  params,
  searchParams,
}: {
  ...
  const [soal, mapelNamaList] = await Promise.all([getSoalGlobalById(id), getSemuaNamaMapelUnik()]);
  if (!soal) notFound();
  ...
  const kembaliHref = `/superadmin/bank-soal/${encodeURIComponent(soal.jenjang ?? "Belum diisi")}/${encodeURIComponent(soal.mapelNama ?? "Tanpa mapel")}`;
  ...
  return (
    <AppShell
      ...
      pageTitle="Ubah Soal (Bank Soal Terpusat)"
    ...
    >
      ...
      <div className="bg-paper-raised border border-rule rounded-xl p-6 max-w-xl">
        <form action="/api/superadmin/bank-soal/update" method="POST">
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="mapelNama" required defaultValue={soal.mapelNama ?? ""} list="mapel-nama-list" className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm" />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="jenjang" required defaultValue={soal.jenjang ?? ""} className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="rekomendasiKelas" defaultValue={soal.rekomendasiKelas ?? ""} className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm" />
            </div>
          </div>
          ...
          {soal.jenis === "JAWABAN_SINGKAT" && (
            <div className="flex flex-col gap-1.5 mb-3">
              ...
              <input name="kunciSingkat" defaultValue={soal.kunciJawaban ?? ""} className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm" />
            </div>
          )}
          ...
          <div className="grid md:grid-cols-3 gap-3 mb-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="topik" defaultValue={soal.topik ?? ""} className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold">Tingkat kesulitan</label>
              <select name="tingkatKesulitan" defaultValue={soal.tingkatKesulitan ?? "sedang"} className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="poinDefault" type="number" defaultValue={soal.poinDefault} className="bg-paper border border-rule rounded-lg px-3 py-2 text-sm" />
            </div>
          </div>

          <div className="flex flex-wrap gap-2">
            <ConfirmSubmitButton size="sm" confirmMessage="Simpan perubahan soal terpusat ini? Perubahan berlaku ke semua sekolah yang memakainya.">Simpan perubahan</ConfirmSubmitButton>
            <a href={kembaliHref} className="text-xs font-semibold text-ink-soft self-center hover:underline">
              Batal
            </a>
          </div>
        </form>
