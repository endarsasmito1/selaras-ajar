...
export default async function PesanOrtuPage() {
  ...
  return (
    <AppShell
      groups={NAV_ORTU}
      activeHref="/ortu/pesan"
      userName={session.nama}
      userRoleLabel={ROLE_LABEL[session.peran]}
      pageTitle="Pesan"
      pageSubtitle="Komunikasi resmi dengan wali kelas anak"
    >
      ...
      <div className="flex flex-col gap-3">
        {inbox.length === 0 && <EmptyState icon="✉" title="Belum ada pesan" hint="Pesan dari wali kelas akan muncul di sini." />}
        {inbox.map((p) => (
